...
final class Island: SKSpriteNode, GameBackgroundSpriteable {
    ...
    static func configureIslandName() -> String {
        let distribuiton = GKRandomDistribution(lowestValue: 1, highestValue: 4)
    ...
    static var randomScaleFactor: CGFloat {
        let distribuiton = GKRandomDistribution(lowestValue: 1, highestValue: 10)
    ...
    static func rotateForRandomAngle() -> SKAction {
        let distribuiton = GKRandomDistribution(lowestValue: 0, highestValue: 360)
        let randomNumber = CGFloat(distribuiton.nextInt())
